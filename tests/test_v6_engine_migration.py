# -*- coding: utf-8 -*-
"""
test_v6_engine_migration.py

验证 Alpha v6 的 run_backtest() 已正确迁移到共享回测引擎。
...
from strategies.alpha_v6_research_strategy_csv import (  # noqa: E402
    run_backtest,
...
class TestRunBacktestOutput:
    ...
    def test_metrics_keys(self, sample_stock_df, sample_benchmark_df):
        ...
        expected_keys = [
            ...
            "buy_hold_total_return", "market_filter_on_ratio", "strategy_exposure_ratio",
        ]
...
class TestEngineConsistency:
    ...
    def test_position_matches_manual(self, sample_stock_df, sample_benchmark_df):
        ...
        # 手动重现引擎逻辑
        signals = compute_alpha_v6_signals(
...
# ---------------------------------------------------------------------------
# 测试：wf_batch_shared 迁移
# ---------------------------------------------------------------------------

class TestWfBatchSharedMigration:
    """验证 wf_batch_shared.py 的 run_one_backtest 和 run_alpha_frame 已迁移到引擎。"""

    def test_run_one_backtest_uses_engine(self, sample_stock_df, sample_benchmark_df):
        """run_one_backtest 应返回与引擎一致的结果。"""
        from scripts.common.wf_batch_shared import run_one_backtest
        from strategies.alpha_v6_research_strategy_csv import compute_alpha_v6_signals
        ...
        # 验证引擎列存在
        assert "equity" in result.columns
        assert "buy_hold_equity" in result.columns
        assert "drawdown" in result.columns

        # 验证 metrics 格式
        assert "strategy_total_return" in metrics
        assert "buy_hold_total_return" in metrics
        assert "excess_vs_buy_hold_total_return" in metrics

    def test_run_alpha_frame_uses_engine(self, sample_stock_df, sample_benchmark_df):
        """run_alpha_frame 应返回包含引擎计算列的 DataFrame。"""
        from scripts.common.wf_batch_shared import run_alpha_frame
        from strategies.alpha_v6_research_strategy_csv import compute_alpha_v6_signals
        ...
        result = run_alpha_frame(
            ...
            turnover_short=10, turnover_long=60,
            divergence_window=20,
            commission=0.0001, sell_tax=0.0005, slippage=0.0,
        )

        # 验证核心列存在
        assert "position" in result.columns
        assert "stock_ret" in result.columns
        assert "cost" in result.columns
        assert "strategy_ret" in result.columns
