# -*- coding: utf-8 -*-
"""
diagnose_alpha_v5_research_strategy_results.py

诊断 Alpha v5 walk-forward 样本外结果，找出策略弱点并给出改进建议。

输入：analyze_alpha_v5_research_walk_forward_results.py 的输出 + walk-forward 原始 CSV
输出：10 个诊断 CSV + 1 个 TXT 建议报告 + 可选 PNG 图表

运行示例：
python scripts\\diagnose_alpha_v5_research_strategy_results.py --run-id exp005_alpha_v5_smoke --input-tag <tag> --no-png
"""
...
DEFAULT_ANALYSIS_DIR = PROJECT_ROOT / "backtests" / "walk_forward_alpha_v5_research_analysis"
DEFAULT_WF_DIR = PROJECT_ROOT / "backtests" / "walk_forward_alpha_v5_research_csv"
...
def list_candidate_files(input_dir: Path, kind: str) -> str:
    candidates = sorted(input_dir.glob(f"wf_alpha_v5_stock_*_{kind}.csv"), key=lambda f: f.stat().st_mtime, reverse=True)
    if not candidates:
        return "  (none)"
...
def find_wf_file(
    ...
    if file_tag:
        pattern = f"wf_alpha_v5_stock_{file_tag}_{kind}.csv"
    ...
    if not allow_fallback:
        raise FileNotFoundError(
            "Alpha v5 diagnosis requires --input-tag to bind an exact walk-forward file group.\n"
            f"Candidate {kind} files:\n{list_candidate_files(input_dir, kind)}"
        )

    files = sorted(input_dir.glob(f"wf_alpha_v5_stock_*top{portfolio_size}_{kind}.csv"), key=lambda f: f.stat().st_mtime, reverse=True)
    if not files:
        files = sorted(input_dir.glob(f"wf_alpha_v5_stock_*_{kind}.csv"), key=lambda f: f.stat().st_mtime, reverse=True)
    if not files:
...
def make_train_vs_test_scatter(gap_df: pd.DataFrame, output_dir: Path) -> Path | None:
    ...
    ax.set_title("Alpha v5: Train vs Test Returns")
    plt.tight_layout()
    path = output_dir / "alpha_v5_diagnosis_train_vs_test_scatter.png"
...
def make_yearly_excess_heatmap(yearly_weakness: pd.DataFrame, output_dir: Path) -> Path | None:
    ...
    ax.set_title("Alpha v5: Yearly Returns")
    ax.set_xlabel("Year")
    ax.set_ylabel("Return")
    plt.tight_layout()
    path = output_dir / "alpha_v5_diagnosis_yearly_returns.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def make_top_draggers_chart(contrib: pd.DataFrame, output_dir: Path, top_n: int) -> Path | None:
    ...
    ax.set_title(f"Alpha v5: Worst {top_n} Stock Contributors")
    plt.tight_layout()
    path = output_dir / "alpha_v5_diagnosis_top_draggers.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def write_recommendations(output_dir: Path, args, summary, yearly_weakness, gap_df, gap_corr,
    ...
    path = output_dir / "alpha_v5_diagnosis_recommendations.txt"
    with open(path, "w", encoding="utf-8") as f:
        f.write("Alpha v5 Strategy Diagnosis Recommendations\n")
        f.write("=" * 60 + "\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="诊断 Alpha v5 walk-forward 策略表现")
    ...
    output_dir = output_root / f"alpha_v5_research_diagnosis_{run_id}"
