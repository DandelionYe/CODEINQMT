# -*- coding: utf-8 -*-
"""
analyze_alpha_v5_research_walk_forward_results.py

分析 Alpha v5 walk-forward 样本外结果。

输入：validate_alpha_v5_research_candidates.py 的输出
输出：11 个 CSV + 1 个 TXT 报告 + 可选 PNG 图表

运行示例：
python scripts\\analyze_alpha_v5_research_walk_forward_results.py --input-tag alpha_v5_ALL_stock_ts20150101_fy2021-2025_avmomentum_reversion_blend_adaptive_momentum_multi_timeframe_momentum_volatility_regime_momentum_mom60_120_rev20_mshort60_mlong250_vol60_brk120_bm000300SH_000905SH_000852SH_bma120_250_top20_limitALL --no-png
"""
...
DEFAULT_INPUT_DIR = PROJECT_ROOT / "backtests" / "walk_forward_alpha_v5_research_csv"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "backtests" / "walk_forward_alpha_v5_research_analysis"
...
def find_one_file(
    ...
    if not allow_fallback:
        raise FileNotFoundError(
            "必须传入 --input-tag 精确指定 alpha v5 walk-forward 文件组。\n"
    ...
    if not files:
        raise FileNotFoundError(f"未找到匹配文件: {input_dir}/wf_alpha_v5_stock_*_{kind}.csv")
    if len(files) > 1:
...
def plot_equity_curve(combined: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("Alpha v5 Walk-Forward Equity Curve")
    ax.set_xlabel("Date")
    ax.set_ylabel("Equity")
    ax.legend(fontsize=8)
    plt.tight_layout()
    path = output_dir / "alpha_v5_wf_analysis_equity_curve.png"
...
def plot_drawdown_curve(combined: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("Alpha v5 Strategy Drawdown")
    ax.set_xlabel("Date")
    ax.set_ylabel("Drawdown")
    plt.tight_layout()
    path = output_dir / "alpha_v5_wf_analysis_drawdown_curve.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def plot_yearly_return_bar(yearly: pd.DataFrame, output_dir: Path) -> Path:
    ...
    plt.tight_layout()
    path = output_dir / "alpha_v5_wf_analysis_yearly_return_bar.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def write_report(output_dir: Path, args, groups, benchmarks, incomplete_year,
    ...
    path = output_dir / "alpha_v5_wf_analysis_report.txt"
    with open(path, "w", encoding="utf-8") as f:
        f.write("Alpha v5 Walk-Forward Analysis Report\n")
        f.write("=" * 60 + "\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="分析 Alpha v5 walk-forward 样本外结果")
