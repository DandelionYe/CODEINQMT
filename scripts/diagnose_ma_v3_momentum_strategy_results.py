# -*- coding: utf-8 -*-
"""
diagnose_ma_v3_momentum_strategy_results.py

诊断 MA v3 walk-forward 样本外结果，找出策略弱点并给出改进建议。

输入：analyze_ma_v3_momentum_walk_forward_results.py 的输出 + walk-forward 原始 CSV
输出：9 个诊断 CSV + 1 个 TXT 建议报告 + 可选 PNG 图表

运行示例：
python scripts\\diagnose_ma_v3_momentum_strategy_results.py --no-png --run-id exp003_test
"""
...
DEFAULT_ANALYSIS_DIR = PROJECT_ROOT / "backtests" / "walk_forward_ma_v3_momentum_analysis"
DEFAULT_WF_DIR = PROJECT_ROOT / "backtests" / "walk_forward_ma_v3_momentum_csv"
...
def normalize_benchmark_entities(benchmarks: list[str]) -> list[str]:
    ...
    for bm in benchmarks:
        if bm in BENCHMARK_ENTITY_MAP:
            result.append(BENCHMARK_ENTITY_MAP[bm])
        else:
            result.append(bm)
    return result
...
def list_candidate_files(input_dir: Path, kind: str) -> str:
    candidates = sorted(input_dir.glob(f"wf_ma_v3_stock_*_{kind}.csv"), key=lambda f: f.stat().st_mtime, reverse=True)
    if not candidates:
        return "  (none)"
...
def find_wf_file(
    ...
    if file_tag:
        pattern = f"wf_ma_v3_stock_{file_tag}_{kind}.csv"
    ...
    if not allow_fallback:
        raise FileNotFoundError(
            "MA v3 diagnosis requires --input-tag to bind an exact walk-forward file group.\n"
            f"Candidate {kind} files:\n{list_candidate_files(input_dir, kind)}"
        )

    files = sorted(input_dir.glob(f"wf_ma_v3_stock_*top{portfolio_size}_{kind}.csv"), key=lambda f: f.stat().st_mtime, reverse=True)
    if not files:
        files = sorted(input_dir.glob(f"wf_ma_v3_stock_*_{kind}.csv"), key=lambda f: f.stat().st_mtime, reverse=True)
    if not files:
...
def load_analysis_tables(analysis_dir: Path) -> dict[str, pd.DataFrame]:
    prefix = "v3_momentum_wf_analysis"
    table_names = [
        "overall_comparison", "excess_comparison", "yearly_comparison", "yearly_excess",
        "selected_frequency", "parameter_frequency", "benchmark_filter_frequency",
        "single_stock_contribution", "combined_daily_returns",
    ]
    tables = {}
...
def build_yearly_weakness(yearly: pd.DataFrame, yearly_excess: pd.DataFrame, exclude_year: int | None) -> pd.DataFrame:
    ...
    for _, s in strat.iterrows():
        ...
        # Check if any benchmark beat strategy
...
def build_parameter_stability(param_freq: pd.DataFrame, detail: pd.DataFrame, exclude_year: int | None) -> pd.DataFrame:
    if param_freq.empty or detail.empty:
        return pd.DataFrame()

    df = filter_exclude_year(detail, exclude_year)
    param_cols = [c for c in ["ma_mid", "ma_long", "momentum_window"] if c in df.columns]
    if not param_cols:
        return pd.DataFrame()
...
def make_train_vs_test_scatter(gap_df: pd.DataFrame, output_dir: Path, top_n: int) -> Path | None:
    ...
    ax.set_title("MA v3: Train vs Test Returns")
    plt.tight_layout()
    path = output_dir / "v3_momentum_diagnosis_train_vs_test_scatter.png"
...
def make_yearly_excess_heatmap(yearly_weakness: pd.DataFrame, output_dir: Path) -> Path | None:
    ...
    ax.set_title("MA v3: Yearly Returns")
    ax.set_xlabel("Year")
    ax.set_ylabel("Return")
    plt.tight_layout()
    path = output_dir / "v3_momentum_diagnosis_yearly_returns.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def make_top_draggers_chart(contrib: pd.DataFrame, output_dir: Path, top_n: int) -> Path | None:
    ...
    ax.set_title(f"MA v3: Worst {top_n} Stock Contributors")
    plt.tight_layout()
    path = output_dir / "v3_momentum_diagnosis_top_draggers.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def write_recommendations(output_dir: Path, args, summary, yearly_weakness, gap_df, gap_corr,
                          bad_contrib, good_contrib, param_stab, selected_rep,
                          table_paths, plot_paths, exclude_year) -> Path:
    path = output_dir / "v3_momentum_diagnosis_recommendations.txt"
    with open(path, "w", encoding="utf-8") as f:
        f.write("MA v3 Strategy Diagnosis Recommendations\n")
        f.write("=" * 60 + "\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="诊断 MA v3 walk-forward 策略表现")
    ...
    output_dir = output_root / f"ma_v3_momentum_diagnosis_{run_id}"
    ...
    param_stab = build_parameter_stability(
        tables.get("parameter_frequency", pd.DataFrame()), detail_raw, exclude_year)
    ...
    if not args.no_png:
        try:
            p = make_train_vs_test_scatter(gap_df, output_dir, args.top_n_chart)
            if p:
