# -*- coding: utf-8 -*-
"""
validate_alpha_v4_research_candidates.py

Alpha v4 walk-forward / 滚动样本外验证：
...
1. 小规模测试：
python scripts\\validate_alpha_v4_research_candidates.py --market SZ --limit 200 --first-test-year 2024 --last-test-year 2024 --portfolio-size 10 --alpha-variant-list pure_momentum,simplified_trend_momentum --momentum-window-list 60,120 --trend-ma-list 120,250 --vol-window-list 60 --breakout-window-list 120 --benchmark-list 000300.SH --benchmark-ma-list 120 --workers 4 --progress-every 50

2. 全市场 walk-forward：
python scripts\\validate_alpha_v4_research_candidates.py --market ALL --security-type stock --first-test-year 2021 --last-test-year 2025 --portfolio-size 20 --alpha-variant-list pure_momentum,simplified_trend_momentum,volatility_adjusted_momentum,breakout_momentum --momentum-window-list 60,120,250 --trend-ma-list 120,250 --vol-window-list 60,120 --breakout-window-list 60,120,250 --benchmark-list 000300.SH,000905.SH,000852.SH --benchmark-ma-list 120,250 --workers 10 --progress-every 100
"""
...
OUTPUT_DIR = PROJECT_ROOT / "backtests" / "walk_forward_alpha_v4_research_csv"
...
def run_alpha_v4_frame(
    ...
    """运行 alpha v4 策略，返回完整结果 DataFrame。"""
    ...
    for year in test_years:
        ...
        for alpha_variant, momentum_window, trend_ma, vol_window, breakout_window in param_combos:
            for (bm_symbol, bm_ma), bm_data in benchmark_cache_data.items():
                required_rows = max(trend_ma, breakout_window, int(bm_ma * 2.5)) + warmup_buffer
                ...
                try:
                    result = run_alpha_v4_frame(
                        train_df, filter_train,
                        alpha_variant, momentum_window, trend_ma, vol_window, breakout_window,
...
def test_selected_for_period(
    ...
    for _, cand in selected.iterrows():
        ...
        trend_ma = int(cand["trend_ma"])
        vol_window = int(cand["vol_window"])
        breakout_window = int(cand["breakout_window"])
        ...
        full_result = run_alpha_v4_frame(
...
def build_tag(args: argparse.Namespace) -> str:
    ...
    return (
        f"alpha_v4_{args.market}_{args.security_type}"
...
def save_outputs(
    ...
    selected_path = OUTPUT_DIR / f"wf_alpha_v4_stock_{tag}_selected_by_year.csv"
    detail_path = OUTPUT_DIR / f"wf_alpha_v4_stock_{tag}_test_detail.csv"
    daily_path = OUTPUT_DIR / f"wf_alpha_v4_stock_{tag}_portfolio_daily.csv"
    period_path = OUTPUT_DIR / f"wf_alpha_v4_stock_{tag}_portfolio_period_summary.csv"
    report_path = OUTPUT_DIR / f"wf_alpha_v4_stock_{tag}_report.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Alpha v4 Walk-Forward Report\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"trend_ma_list: {args.trend_ma_list}\n")
        f.write(f"vol_window_list: {args.vol_window_list}\n")
        f.write(f"breakout_window_list: {args.breakout_window_list}\n")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Alpha v4 walk-forward 样本外验证")
    ...
    parser.add_argument("--trend-ma-list", default="120,250")
    parser.add_argument("--vol-window-list", default="60,120")
    parser.add_argument("--breakout-window-list", default="60,120,250")
...
def main() -> None:
    ...
    param_combos = build_variant_param_combos(
        ...
        parse_int_list(args.trend_ma_list),
        parse_int_list(args.vol_window_list),
        parse_int_list(args.breakout_window_list),
    )
