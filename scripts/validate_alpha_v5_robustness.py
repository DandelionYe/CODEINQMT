# -*- coding: utf-8 -*-
"""
validate_alpha_v5_robustness.py

Alpha v5 稳健性验证模块：判断 full-run walk-forward 结果是否足够稳健，
...
运行示例：
python scripts/validate_alpha_v5_robustness.py --input-tag <tag> --run-id exp005_alpha_v5_full --no-png
"""
...
def load_input_files(wf_dir: Path, input_tag: str) -> dict[str, pd.DataFrame]:
    prefix = f"wf_alpha_v5_stock_{input_tag}"
    required = {
...
def write_report(
    ...
    path = output_dir / "alpha_v5_robustness_report.txt"
    with open(path, "w", encoding="utf-8") as f:
        f.write("Alpha v5 Robustness Validation Report\n")
        f.write("=" * 70 + "\n\n")
...
def make_equity_chart(daily: pd.DataFrame, exclude_years: list[int], output_dir: Path) -> Path:
    ...
    ax.set_title("Alpha v5: Equity Curve (Full vs Exclude Years)")
    ax.set_xlabel("Date")
...
def make_loyo_chart(loyo_df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("Alpha v5: Leave-One-Year-Out Returns")
...
def make_yearly_chart(yearly_df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.axhline(0, color="black", linewidth=0.5)
    ax.set_title("Alpha v5: Yearly Contribution")
    ax.set_xlabel("Year")
...
def make_variant_chart(variant_df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("Alpha v5: Variant Stability (Avg Test Return)")
...
def make_concentration_chart(concentration_df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.axvline(0, color="black", linewidth=0.5)
    ax.set_title("Alpha v5: Top 20 Stock Contributors")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Alpha v5 稳健性验证")
    ...
    parser.add_argument("--run-id", default="exp005_alpha_v5_full")
    ...
    output_dir = output_root / f"alpha_v5_robustness_{args.run_id}"
    ...
    csv_tables = {
        "alpha_v5_robustness_summary": summary_df,
        "alpha_v5_robustness_leave_one_year_out": loyo_df,
        "alpha_v5_robustness_exclude_years": exclude_df,
        "alpha_v5_robustness_yearly_contribution": yearly_df,
        "alpha_v5_robustness_benchmark": benchmark_df,
        "alpha_v5_robustness_variant": variant_df,
        "alpha_v5_robustness_parameters": param_df,
        "alpha_v5_robustness_concentration": concentration_df,
        "alpha_v5_robustness_train_test": train_test_df,
        "alpha_v5_robustness_decision": pd.DataFrame(gates),
    }
