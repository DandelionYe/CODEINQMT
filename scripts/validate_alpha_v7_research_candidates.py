# -*- coding: utf-8 -*-
"""
validate_alpha_v7_research_candidates.py

Alpha v7 walk-forward / 滚动样本外验证：
...
1. 小规模测试：
python scripts\\validate_alpha_v7_research_candidates.py --market SZ --limit 200 --first-test-year 2024 --last-test-year 2024 --portfolio-size 10 --alpha-variant-list short_term_reversal,low_volatility --reversal-window-list 5,10 --vol-window-list 60 --benchmark-list 000300.SH --benchmark-ma-list 120 --workers 4 --progress-every 50

2. 全市场 walk-forward：
python scripts\\validate_alpha_v7_research_candidates.py --market ALL --security-type stock --first-test-year 2021 --last-test-year 2025 --portfolio-size 20 --alpha-variant-list short_term_reversal,low_volatility,turnover_reversal,volume_price_divergence --reversal-window-list 5,10,20 --vol-window-list 20,60,120 --turnover-short-list 10 --turnover-long-list 60 --divergence-window-list 10,20,60 --benchmark-list 000300.SH,000905.SH,000852.SH --benchmark-ma-list 120,250 --workers 10 --progress-every 100
"""
...
OUTPUT_DIR = PROJECT_ROOT / "backtests" / "walk_forward_alpha_v7_research_csv"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

CFG = WFValidateConfig(
    alpha_version="alpha_v7",
    output_dir_name=str(OUTPUT_DIR),
    file_prefix="wf_alpha_v7_stock",
    report_title="Alpha v7 Walk-Forward Report (expression layer)",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Alpha v7 walk-forward 样本外验证（表达式层）")
...
def main() -> None:
    ...
    if args.workers > 1:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as executor:
            futures = {
                executor.submit(train_one_stock_all_years, task, compute_alpha_v7_signals): task[0]["symbol"]
                for task in tasks
            }
