# -*- coding: utf-8 -*-
"""
analyze_ma_v3_momentum_walk_forward_results.py

分析 MA v3 walk-forward 样本外结果。

输入：validate_ma_v3_momentum_candidates.py 的输出
输出：9 个 CSV + 1 个 TXT 报告 + 可选 PNG 图表

运行示例：
python scripts\\analyze_ma_v3_momentum_walk_forward_results.py
python scripts\\analyze_ma_v3_momentum_walk_forward_results.py --no-png
"""
...
DEFAULT_INPUT_DIR = PROJECT_ROOT / "backtests" / "walk_forward_ma_v3_momentum_csv"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "backtests" / "walk_forward_ma_v3_momentum_analysis"
...
def find_one_file(
    ...
    if not allow_fallback:
        raise FileNotFoundError(
            "必须传入 --input-tag 精确指定 MA v3 walk-forward 文件组。\n"
            f"候选 {kind} 文件:\n{list_candidate_files(input_dir, kind)}"
        )

    # Fallback: try portfolio_size, then any
    ...
    if not files:
        raise FileNotFoundError(f"未找到匹配文件: {input_dir}/wf_ma_v3_stock_*_{kind}.csv")
    if len(files) > 1:
...
def analyze_parameter_frequency(groups: dict) -> pd.DataFrame:
    ...
    param_cols = []
    for c in ["ma_mid", "ma_long", "momentum_window"]:
...
def analyze_benchmark_filter_frequency(groups: dict) -> tuple[pd.DataFrame, dict]:
    ...
    freq_cols = []
    for c in ["benchmark", "benchmark_ma"]:
        if c in selected.columns:
            freq_cols.append(c)
...
def plot_equity_curve(combined: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("MA v3 Walk-Forward Equity Curve")
    ax.set_xlabel("Date")
    ax.set_ylabel("Equity")
    ax.legend(fontsize=8)
    plt.tight_layout()
    path = output_dir / "v3_momentum_wf_analysis_equity_curve.png"
...
def plot_drawdown_curve(combined: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("MA v3 Strategy Drawdown")
    ax.set_xlabel("Date")
    ax.set_ylabel("Drawdown")
    plt.tight_layout()
    path = output_dir / "v3_momentum_wf_analysis_drawdown_curve.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def plot_yearly_return_bar(yearly: pd.DataFrame, output_dir: Path) -> Path:
    ...
    plt.tight_layout()
    path = output_dir / "v3_momentum_wf_analysis_yearly_return_bar.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    return path
...
def write_report(output_dir: Path, args, groups, benchmarks, incomplete_year,
                 ...
                 param_freq, benchmark_filter_freq, ratio_stats, contribution,
                 table_paths, plot_paths) -> Path:
    path = output_dir / "v3_momentum_wf_analysis_report.txt"
    with open(path, "w", encoding="utf-8") as f:
        f.write("MA v3 Walk-Forward Analysis Report\n")
        f.write("=" * 60 + "\n\n")
        ...
        # Ratio stats
        if ratio_stats:
            f.write("Volatility/Drawdown Stats:\n")
            for col, stats in ratio_stats.items():
                f.write(f"  {col}: mean={stats['mean']:.4f}, median={stats['median']:.4f}\n")
            f.write("\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="分析 MA v3 walk-forward 样本外结果")
