# -*- coding: utf-8 -*-
"""
diagnose_alpha_v7_research_strategy_results.py

诊断 Alpha v7 walk-forward 样本外结果，找出策略弱点并给出改进建议。

输入：analyze_alpha_v7_research_walk_forward_results.py 的输出 + walk-forward 原始 CSV
输出：10 个诊断 CSV + 1 个 TXT 建议报告 + 可选 PNG 图表

运行示例：
python scripts\\diagnose_alpha_v7_research_strategy_results.py --run-id exp007_alpha_v7_full --input-dir backtests/walk_forward_alpha_v7_research_csv --analysis-dir backtests/walk_forward_alpha_v7_research_analysis --no-png
"""
...
CFG: WFReportConfig = make_v7_config(PROJECT_ROOT)
...
def main() -> None:
    ...
    parser.add_argument("--input-dir", default="", help="walk-forward 输出目录（优先于 --walk-forward-dir）")
    ...
    wf_dir = resolve_path(args.input_dir if args.input_dir else args.walk_forward_dir)
